...
//!
//! Uses a dedicated manager thread that owns the `HotkeyManager`.
//! Commands (register/unregister) are sent via mpsc channel.
//! For UI key capture, a separate `KeyboardListener` is created on-demand
//! and polled from a dedicated recording thread (see recording.rs).
...
pub struct HandyKeysState {
    ...
    pub(super) recording_listener: Mutex<Option<KeyboardListener>>,
    pub(super) is_recording: AtomicBool,
    pub(super) recording_binding_id: Mutex<Option<String>>,
    pub(super) recording_running: Arc<AtomicBool>,
}
